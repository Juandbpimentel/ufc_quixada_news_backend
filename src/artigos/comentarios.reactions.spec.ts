...
describe('ComentariosService reactions', () => {
  ...
  test('createCommentReaction should create when none exists', async () => {
    ...
    mockPrisma.comentarioReacao.create.mockResolvedValue({
      id: 1,
      tipo: 'GOSTEI',
    });
    const res = await svc.createCommentReaction(2, 10, 'GOSTEI' as any);
    expect(mockPrisma.comentarioReacao.create).toHaveBeenCalledWith({
      data: { usuarioId: 2, comentarioId: 10, tipo: 'GOSTEI' },
    });
  ...
  test('createCommentReaction should throw NotFound when comment does not exist', async () => {
    ...
    await expect(
      svc.createCommentReaction(2, 999, 'GOSTEI' as any),
    ).rejects.toThrow('Comentário não encontrado');
  });

  test('createCommentReaction should update when exists', async () => {
    mockPrisma.comentario.findUnique = jest.fn().mockResolvedValue({ id: 10 });
    mockPrisma.comentarioReacao.findFirst.mockResolvedValue({
      id: 5,
      tipo: 'GOSTEI',
    });
    mockPrisma.comentarioReacao.update.mockResolvedValue({
      id: 5,
      tipo: 'NAO_GOSTEI',
    });
    const res = await svc.createCommentReaction(2, 10, 'NAO_GOSTEI' as any);
    expect(mockPrisma.comentarioReacao.update).toHaveBeenCalledWith({
      where: { id: 5 },
      data: { tipo: 'NAO_GOSTEI' },
    });
  ...
  test('deleteCommentReactionByUser should delete when present', async () => {
    ...
    expect(mockPrisma.comentarioReacao.delete).toHaveBeenCalledWith({
      where: { id: 8 },
    });
